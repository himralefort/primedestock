...
def generate_proforma(order_id):
    try:
        ...
        elements.append(Paragraph("FACTURE PROFORMA", styles['Title']))
        elements.append(Paragraph("Destockage Alimentaire Professionnel", styles['Header']))
        elements.append(Paragraph("123 Rue du Commerce, 75000 Paris", styles['Normal']))
        elements.append(Paragraph("Tél: 01 23 45 67 89 | contact@destockage-pro.com", styles['Normal']))
        elements.append(Spacer(1, 30))
        
        # Informations client et entreprise
        client_info = [
            [Paragraph("<b>Facture N°</b>", styles['Bold']), order_id],
            [Paragraph("<b>Date</b>", styles['Bold']), order['date']],
            [Paragraph("<b>Client</b>", styles['Bold']), order.get('user', 'Invité')],
            [Paragraph("<b>Méthode de paiement</b>", styles['Bold']), order.get('payment_method', 'Non spécifié')],
        ]
        
        company_info = [
            [Paragraph("<b>SIRET</b>", styles['Bold']), "123 456 789 00012"],
            [Paragraph("<b>TVA Intracom.</b>", styles['Bold']), "FR12345678901"],
            [Paragraph("<b>RCS</b>", styles['Bold']), "Paris B 123 456 789"],
        ]
        
        info_table = Table([client_info, company_info], colWidths=[2.5*inch, 2.5*inch])
        ...
        elements.append(Paragraph("<b>DÉTAIL DE LA COMMANDE</b>", styles['SectionTitle']))
        ...
        # Totaux
        items_data.append(["", "", "<b>Sous-total</b>", f"<b>{order['total']:.2f}€</b>"])
        items_data.append(["", "", "<b>TVA (20%)</b>", f"<b>{order['total'] * 0.2:.2f}€</b>"])
        items_data.append(["", "", "<b>Total TTC</b>", f"<b>{order['total'] * 1.2:.2f}€</b>"])
        
        items_table = Table(items_data, colWidths=[3*inch, 1.2*inch, 1*inch, 1.2*inch])
        ...
        payment_info = [
            ["<b>Date d'échéance</b>", (datetime.now() + timedelta(days=30)).strftime('%d/%m/%Y')],
            ["<b>Mode de paiement</b>", "Virement bancaire"],
            ["<b>IBAN</b>", "FR76 1234 5678 9123 4567 8912 345"],
            ["<b>BIC</b>", "ABCDEFGH123"],
        ]
        ...
        # Mentions légales
        elements.append(Paragraph("En cas de retard de paiement, seront exigibles, conformément à l'article L. 441-6 du code de commerce, une indemnité forfaitaire pour frais de recouvrement de 40€.", styles['Normal']))
        elements.append(Spacer(1, 10))
        elements.append(Paragraph("TVA non applicable - article 293 B du CGI", styles['Normal']))
        
        # Pied de page
        elements.append(Spacer(1, 30))
        elements.append(Paragraph("Destockage Alimentaire Professionnel - SAS au capital de 50 000€", styles['Footer']))
        elements.append(Paragraph("RCS Paris B 123 456 789 - N° TVA Intracom.: FR12345678901", styles['Footer']))
        ...
        response.headers['Content-Disposition'] = f'attachment; filename=Facture_Proforma_{order_id}.pdf'
        ...
        app.logger.error(f"Erreur lors de la génération de la facture: {str(e)}")
        flash("Une erreur est survenue lors de la génération de la facture", "error")
        return redirect(url_for('confirmation', order_id=order_id))
